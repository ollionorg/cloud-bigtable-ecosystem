...
type MockResponseHandler struct {
	...
	mockBuildResponseRow func(rowMap map[string]interface{}, query rh.QueryMetadata, cmd []*message.ColumnMetadata, mapKeyArray []string, lastRow bool) (message.Row, error)
	mock.Mock
}
...
func (m *MockResponseHandler) BuildResponseRow(rowMap map[string]interface{}, query rh.QueryMetadata, cmd []*message.ColumnMetadata, mapKeyArray []string, lastRow bool) (message.Row, error) {
	return m.mockBuildResponseRow(rowMap, query, cmd, mapKeyArray, lastRow)
}
...
func TestExecuteBigtableQuery(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			btc := NewBigtableClient(map[string]*bigtable.Client{"fake-instance": {}}, map[string]*bigtable.AdminClient{"fake-instance": {}}, zap.NewNop(), mockClient, BigtableConfig{}, mockResponseHandler, nil, nil)
...
func TestSelect_ConstructRequestValues(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			value: "test-string",
			expected: &btpb.Value{
				Kind: &btpb.Value_StringValue{StringValue: "test-string"},
			...
			expected: &btpb.Value{
				Kind: &btpb.Value_ArrayValue{
					ArrayValue: &btpb.ArrayValue{
						Values: []*btpb.Value{
							{
								Kind: &btpb.Value_StringValue{StringValue: "test1"},
								Type: &btpb.Type{Kind: &btpb.Type_StringType{}},
							},
							{
								Kind: &btpb.Value_StringValue{StringValue: "test2"},
								Type: &btpb.Type{Kind: &btpb.Type_StringType{}},
							},
						},
					},
				...
				Type: &btpb.Type{Kind: &btpb.Type_ArrayType{
					ArrayType: &btpb.Type_Array{ElementType: &btpb.Type{Kind: &btpb.Type_StringType{}}}},
				},
			},
			expectErr: false,
		},
...
func TestSelectStatement(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			btc := NewBigtableClient(map[string]*bigtable.Client{"test-instance": {}}, map[string]*bigtable.AdminClient{"test-instance": {}}, zap.NewNop(), mockClient, BigtableConfig{}, mockResponseHandler, nil, nil)
