...
func parseOrderByFromSelect(input cql.IOrderSpecContext) OrderBy {
	...
	count := input.GetChildCount()

	// count := element.GetChildCount()
	for i := 0; i < count; i++ {

		element := input.OrderSpecElement()
		fmt.Printf("element.GetText(): %v\n", element.GetText())
		child := input.OrderSpecElement()

		fmt.Println(i, child, child.GetPayload())
	}
	print(count)
	object := input.OrderSpecElement().OBJECT_NAME()
...
func processNonFunctionColumn(t *Translator, columnMetadata schemaMapping.SelectedColumns, tableName string, keySpace string, columnFamily string, columns []string, structs []AsKeywordMeta, isGroupBy bool) ([]string, []AsKeywordMeta, string, bool, error) {
	...
	if !ok {
		return nil, nil, "", false, fmt.Errorf("Column metadata not found")
	}
	if !columnMetadata.IsWriteTimeColumn {
		dataType = colMeta.CQLType
	}
