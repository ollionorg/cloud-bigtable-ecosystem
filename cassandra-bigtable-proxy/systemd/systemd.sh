...
required_vars=("BIGTABLE_CONFIG_TABLE" "BIGTABLE_DB_NAME" "GCP_PROJECT_ID" "BIGTABLE_INSTANCE")
...
export BIGTABLE_CONFIG_TABLE
export BIGTABLE_DB_NAME
...
cd "$GO_PROJECT_ROOT"
GOOS=linux GOARCH=amd64 go build -o cassandra-to-bigtable-proxy -buildvcs=false
...
cp "$GO_PROJECT_ROOT/cassandra-to-bigtable-proxy" /usr/local/bin/
chmod 755 /usr/local/bin/cassandra-to-bigtable-proxy

# Update the environment variables in the env_cassandra_proxy file
echo "Updating environment variables..."
cat << EOF > /etc/systemd/system/env_cassandra_proxy
BIGTABLE_CONFIG_TABLE="${BIGTABLE_CONFIG_TABLE}"
BIGTABLE_DB_NAME="${BIGTABLE_DB_NAME}"
...
[Unit]
Description=Cassandra to Bigtable Proxy Service

[Service]
ExecStart=/usr/local/bin/cassandra-to-bigtable-proxy
